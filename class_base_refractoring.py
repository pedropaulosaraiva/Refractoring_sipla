...
################################# Métodos Novos
    ...
    def getCirAT_MT_DB(self, nomeSE_AT):
        ...
        try:
            ...
            ct_at = self.DataBaseConn.getSQLDB("CTAT", "SELECT DISTINCT nom FROM ctat WHERE nom LIKE '"+
                                               nomeSE_AT+"%' AND NOT (nom LIKE '%2' OR nom LIKE '%3' OR nom LIKE '%4') ORDER BY nom")
    ...
    def getSE_MT_AL_DB(self, nomeSE_MT):
        ...
        try:
            ...
            ct_mt = self.DataBaseConn.getSQLDB("CTMT","SELECT DISTINCT nom, cod_id FROM ctmt WHERE sub = '"+
